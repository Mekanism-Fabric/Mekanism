package mekanism.inventory.slot;
...
import mekanism.inventory.container.slot.ContainerSlotType;
import mekanism.inventory.container.slot.InventoryContainerSlot;
import mekanism.inventory.container.slot.SlotOverlay;
...
public class BasicInventorySlot implements IInventorySlot {
    ...
    private void setStack(ItemStack stack, boolean validateStack) {
        if (stack.isEmpty()) {
            ...
            throw new RuntimeException("Invalid stack for slot: " + stack.getItem().getDescription() + " " + stack.getCount() + " " + stack.getTag());
        }
        onContentsChanged();
    }
